...
class MADE_Full(nn.Module):
    def __init__(self, input_size, hidden_size, n_hidden, cond_label_size=None, activation='relu',
        ...
        # create masks
        masks, self.input_degrees = create_masks_pmu(int(input_size / 3), hidden_size, n_hidden, input_order,
                                                     input_degrees)
...
class MAF_Full(nn.Module):
    def __init__(self, n_blocks, input_size, hidden_size, n_hidden, cond_label_size=None, activation='relu',
        ...
        self.input_degrees = None
        for i in range(n_blocks):
            modules += [MADE_Full(input_size, hidden_size, n_hidden, cond_label_size, activation, input_order,
                                  self.input_degrees)]
            self.input_degrees = modules[-1].input_degrees.flip(0)
            modules += batch_norm * [BatchNorm(input_size)]
